...
import (
	...
	"github.com/onosproject/config-adapter/internal/pkg/version"
	"github.com/onosproject/config-adapter/pkg/diagapi"
	"github.com/onosproject/config-adapter/pkg/gnmi"
	synchronizer "github.com/onosproject/config-adapter/pkg/synchronizer"
	"github.com/onosproject/config-adapter/pkg/target"
...
var (
	...
	onosConfigAddr       = flag.String("onos_config_addr", "", "If specified, pull initial state from onos-config at this address")
	onosConfigTarget     = flag.String("onos_config_target", "connectivity-service-v4", "Target to use when pulling from onos-config")
...
var log = logging.GetLogger("reference-adapter")
...
// Synchronize and eat the error. This lets onos-config know we applied the
...
func main() {
	...
	diagapi.StartDiagnosticAPI(s, *onosConfigAddr, *onosConfigTarget, *diagsPort)
